...
class HeapSnapshot:
        ...
        for node in self.nodes:
            for _ in range(node.edge_count):
                if edge_idx < len(self.edges):
                    ...
                    target_node_idx = edge.to_node // len(self.snapshot.get('meta', {}).get('node_fields', []))
                    if target_node_idx < len(self.nodes):
                        self.retained_by[self.nodes[target_node_idx].node_id].append((node.node_id, edge))
                    edge_idx += 1
    ...
    def find_retaining_path(self, node_id: int, max_depth: int = 10) -> List[Tuple[HeapNode, HeapEdge]]:
        """
        ...
        while queue:
            ...
            if current_node.node_type == 'synthetic' and current_node.name == '(GC roots)':
                return path
...
async def capture_snapshot(cdp_client, output_path: Optional[str] = None) -> HeapSnapshot:
    """
...
def detect_leaks(snapshots: List[HeapSnapshot], threshold_mb: float = 1.0) -> pd.DataFrame:
    """
        ...
        if all(d > 0 for d in deltas):
            ...
            if total_growth > threshold_bytes:
                ...
                leaks.append({
                    ...
                    'growth_per_snapshot_mb': (total_growth / len(deltas)) / (1024 * 1024),
