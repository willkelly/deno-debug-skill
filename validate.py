...
from visualize import flamegraph, memory_growth_chart
...
async def validate_heap_snapshot(client: CDPClient):
    ...
    try:
        ...
        print_success(
            f"Parsed: {len(snapshot.nodes)} nodes, {len(snapshot.edges)} edges"
        )
...
async def validate_cpu_profile(client: CDPClient):
    ...
    try:
        ...
        print_success(
            f"Parsed: {len(profile.nodes)} nodes, {profile.total_samples} samples"
        )
...
async def validate_comparison(client: CDPClient):
    ...
    try:
        ...
        if not comparison.empty:
            ...
            print(
                f"    Top growth: {comparison.iloc[0]['name']} (+{comparison.iloc[0]['size_delta']} bytes)"
            )
        else:
...
async def run_validation():
    ...
    try:
        ...
        # Test heap snapshots
        await validate_heap_snapshot(client)
