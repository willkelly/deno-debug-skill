...
function invalidateCache(userId: string): void {
  ...
  for (const [cachedUserId, _] of permissionCache.entries()) {
    if (cachedUserId.startsWith(userId.substring(0, 3))) {
      ...
      console.log(
        `  ⚠️ Also invalidated cache for ${cachedUserId} (prefix match)`,
      );
    }
  }
}
...
async function simulateBuggyWorkflow(): Promise<void> {
  ...
  console.log(
    `User1 name now: ${user1.username} (expected: alice, got: ${user1.username})`,
  );
  ...
  console.log(
    `User-001 still has write: ${user1HasWrite} (cache was invalidated!)`,
  );
...
function startServer() {
  ...
  console.log(
    "  1. Set conditional breakpoint: break when session.corrupted === true",
  );
  ...
  Deno.serve({ port: 8003 }, async (req) => {
