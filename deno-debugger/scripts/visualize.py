...
def call_tree_visualization(
    ...
    for node in nodes:
        ...
        rect = Rectangle(
            (x, y), width, 0.8, facecolor=color, edgecolor="black", linewidth=1
        )
        ax.add_patch(rect)
    ...
    for item in tree:
        ...
        if children and depth < max_depth:
            ...
            for child in children:
                ...
                child_width = (
                    (child_samples / total_child_samples * x_width)
                    if total_child_samples > 0
                    else x_width / len(children)
                )

                _flatten_tree(
                    [child], nodes, depth + 1, child_x, child_width, max_depth
                )
                child_x += child_width
...
def investigation_timeline_chart(
    breadcrumbs, output_path: str, title: str = "Investigation Timeline"
) -> str:
    """
